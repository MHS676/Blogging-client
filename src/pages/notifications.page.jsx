...
const Notifications = () => {
  ...
  const fetchNotifications = ({ page, deleteDocCounts = 0 }) => {

    axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/notifications", {
      page, filter, deleteDocCounts
    }, {
      headers: {
    ...
    <div>
      ...
      </div>
      {
        ...
        <>
          ...
          <LoadMoreDataBtn state={notifications} fetchDataFun={fetchNotifications} additionalParam={{ deleteDocCount: notifications.deleteDocCount }} />
        </>
      }
    </div>
  )
