...
const CommentCard = ({ index, leftVal, commentData }) => {
  ...
  if (commentsArr[currentIndex].children.length) {
    ...
    let parentIndex = getParentIndex(); // Get the index of the parent comment
    ...
    if (commentsArr[index + 1]) {
        ...
        // If no more replies are loaded, check if we should still show the button
        if (parentIndex) {
            if ((index - parentIndex) < commentsArr[parentIndex].children.length) {
                return button;
            }
    ...
    // Return nothing if no more replies can be loaded
    return null;
};
    ...
    <div className=' w-full' style={{ paddingLeft: `${leftVal * 10}px` }}>
      <div className=' my-5 p-6 rounded-md border border-grey '>
...

            {
              ...
              <button className='p-2 px-3 rounded-md border border-grey ml-auto hover:bg-red/30 flex items-center' onClick={deleteComment} >
