        ...
        <div className='flex gap-2 max-lg:mb-6 max-lg:pb-6 border-grey max-lg:border-b'>
            {
                Object.keys(stats).map((key, i) => {
                    ...
                    <div key={i} className={'flex flex-col items-center w-full h-full justify-center p-4 px-6' + (i != 0 ? "border-grey border-1 " : "") }>
                            ...
                            <p className='max-lg:to-dark-grey capitalize'>{key.split("_")[1]}</p>
                        </div> : ""
                })
            }
        </div>
...
export const ManageDarftBlogPost = ({ blog }) => {
...
const deleteBlog = (blog, access_token, target) => {
  ...
  .then(({ data }) =>{
    ...
    setStateFunc(preVal =>{
      ...
      let { deleteDocCount: deletedDocCount, totalDoc, results } = preVal;
      ...
      return { ...preVal, totalDoc: totalDoc - 1, deleteDocCount: deletedDocCount + 1 }
